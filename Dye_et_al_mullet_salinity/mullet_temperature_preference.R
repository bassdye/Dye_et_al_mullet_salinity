# Author: Bass Dye
# Contact: bass.dye@nioz.nl or bass.d.dye@gmail.com
# Created: March 25th, 2022
# Last revision: December 6th, 2023

#### OVERVIEW ####
# The following R script provides the steps to statistically analyze (i.e. reproduce results from Dye et al. 202X) 
# the compartmental usage of the thinlip mullet within an experimental water temperature preference chamber
# The script is based on pcTEMP (https://github.com/bassdye/pcTEMP) and follows methods in Aebischer et al. (1993) and Schram et al. (2013)
# 1) Statistically analyze the data for non random use of the preference chamber
# 2) Conduct MANOVA to test for differences in compartment usage during the different observational phases and salinity treatments
# 3) Rank the temperature compartments (i.e. most preferred to least preferred)
# 4) Statistically test the preference rankings
...
# Dye, B., Kuijten, S., Tulp, I., Blom, E., Poos, J.J., Schram, E., 202X. Influence of salinity on the thermal preference of juvenile thinlip mullet, Chelon ramada (submitted)
...
temp_range <-  c("27*C", "24*C", "21*C", "18*C", "15*C")

#### LOAD IN DATA ####
# Function to read in multiple sheets within one excel workbook file 
...
# Takes a bit of time; be patient
...
#### ORGANIZE DATA FOR VARIOUS ANALYSES ####
...
# C1: 27 degrees: compartment 4
# C2: 24 degrees: compartments 3 and 5
# C3: 21 degrees: compartments 2 and 6
# C4: 18 degrees: compartments 1 and 7
# C5: 15 degrees: compartment 8
if(exists("number_treatments")) {
...
# The results of the analyses do not depend upon the choice of denominator. In this way, the unit sum constraint on the composition is broken (Aitchison, 1986)
...
# Offset log-ratios of usage with log-ratios of availability of temperature zones. 
# I.e. when the temperature gradient is installed within the chamber, 1/8 of the chamber is the lowest (and highest) 
# temperature of the gradient while the intermediate temperatures are 1/4 of the chamber
...
# Select which experimental phase (non-gradient = "I", acute - "II", final - "III")  
non_test_phase <- "I"
non_test_treatment <- "5" 
...
#### ASSESS DIFFERENCES BETWEEN PHASES AND TREATMENTS USING MANOVA ####
# The matrix of log-ratios can be analysed using MANOVA, for example to assess evidence of treatment effects
...
summary(manova(cbind(LRA1, LRA2, LRA3, LRA4) ~ phase + treatment + Error(factor(subject)), data = prop_dat))
...
res_phase_test_p <- list()
res_phase_test_f <- list()
...
for(i in seq(along = res_phase[-1])) {
  ...
  res_phase_test_p[i] <- tmp[["Error: Within"]][["stats"]][1,6] # Save Wilks for phase
  res_phase_test_f[i] <- tmp[["Error: Within"]][["stats"]][1,3] # Save Wilks for phase
...
tmp <- summary(manova(cbind(LRA1, LRA2, LRA3, LRA4) ~ phase + treatment + Error(factor(subject)), data = testing_data),
               test = "Wilks")
...
perm_phase_p_value <- 2*(min(sum(res_phase <= obs_phase), sum(res_phase >= obs_phase)) / (nperm+1)); perm_phase_p_value
obs_phase_p_value <- tmp[["Error: Within"]][["stats"]][1,6]; obs_phase_p_value
perm_treatment_p_value <- 2*(min(sum(res_treatment <= obs_treatment), sum(res_treatment >= obs_treatment)) / (nperm+1)); perm_treatment_p_value
obs_treatment_p_value <- tmp[["Error: Within"]][["stats"]][2,6]; obs_treatment_p_value
...
# Once overall evidence of non-random use has been assessed,
...
# Select for non-gradient, acute, or final observational phase; choose (I,II,III) and treatment (25, 5) accordingly
test_phase <- "II" 
test_treatment <- "5" 
...
# First, test for normality via the Shapiro-Wilk's method
# A p value > 0.05 implies the distribution of data is not significantly different from a normal distribution (i.e. normally distributed)
...
# Then, test the statistical significance of pairwise temperature zone comparisons (i.e. temperature preference)
# The p values are calculated from the test statistic (T score = mean / se; Aebischer et al. 1993)
...
# 2) Return to "RANKING TEMPERATURE PREFERENCE" section (line 354) to continue testing temperature preferences during the other observational phases and treatments
rm(list=setdiff(ls(), c("prop_dat", "number_treatments", "temp_range")))
